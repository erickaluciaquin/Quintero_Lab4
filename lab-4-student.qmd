---
title: "STAT 210 Lab 4 Ericka Quintero"
...
```{r}
#| label: load-data
buoydata <- read_table("buoy_2024.txt") # reads whitespace-separated files
# read.table(buoydata)
```
...
# buoy <- read_table("buoy_2024.txt.webloc") # reads whitespace-separated files
# spec(buoy)
```
...
```{r}
#| label: clean-names
# buoy |> janitor::clean_names() # snake_case argument, cleans variable names
```
...
write.csv(
  "buoy_2024.txt.webloc", file = "data-raw/buoy_update.csv")
```
...
# buoydata <- read_csv(
   # "data-raw/buoy_update.csv", col_types = list(.default = # col_double())
```
